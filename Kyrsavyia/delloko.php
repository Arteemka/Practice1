...
<div>
 ...
 mysql_select_db($db)//параметр в скобках ("имя базы, с которой соединяемся")
  ...
  if(isset($_POST['button-no'])) {
    ...
    $query ="DELETE FROM lokomotivi WHERE id_lok= '$id'";
    $result = mysql_query( $query) or die("Ошибка " . mysql_error($link)); 
    $sql = "SET @i :=0";
    $s = "UPDATE lokomotivi SET  id_lok = (@i := @i + 1)";
    $g = "ALTER TABLE lokomotivi AUTO_INCREMENT = 1";
   ...
   // $m = mysql_query( $h) or die("Ошибка " . mysql_error($link)); 
    mysql_close($link);
    exit('<meta http-equiv="refresh" content="0; url=http://dsdas/Kyrsavyia/delloko.php" />');
...
<script src="./js/dellokomotiv.js"></script>
