...
<div>
           ...
           //   or die("<p>Ошибка выбора базы данных! ". mysql_error() . "</p>");
            ...
            if($result)
            {
                ...
                for ($i = 0 ; $i < $rows ; ++$i)
                {
                    ...
                    echo "<tr class='num'> ";
                        for ($j = 1 ; $j < 4 ; $j+=2) echo "<td>$row[$j]</td>";
                    echo "</a></tr>";
 ...
 if(isset($_POST['button-no'])) {
    ...
    $query ="DELETE FROM teh_brigad WHERE id_brig = '$id'";
    $result = mysql_query( $query) or die("Ошибка " . mysql_error($link)); 
    $sql = "SET @i :=0";
    $s = "UPDATE teh_brigad SET  id_brig = (@i := @i + 1)";
    $g = "ALTER TABLE teh_brigad AUTO_INCREMENT = 1";
   ...
   // $m = mysql_query( $h) or die("Ошибка " . mysql_error($link)); 
    mysql_close($link);
    exit('<meta http-equiv="refresh" content="0; url=http://dsdas/Kyrsavyia/deletebrigada.php" />');
...
<script src="./js/delbrig.js"></script>
