...
<div>
 ...
 mysql_select_db($db)//параметр в скобках ("имя базы, с которой соединяемся")
  ...
  if(isset($_POST['button-no'])) {
    ...
    $query ="DELETE FROM otmen_reis WHERE id_r= '$id'";
    $result = mysql_query( $query) or die("Ошибка " . mysql_error($link)); 
    $sql = "SET @i :=0";
    $s = "UPDATE otmen_reis SET  id_r= (@i := @i + 1)";
    $g = "ALTER TABLE otmen_reis AUTO_INCREMENT = 1";
   ...
   // $m = mysql_query( $h) or die("Ошибка " . mysql_error($link)); 
    mysql_close($link);
    exit('<meta http-equiv="refresh" content="0; url=http://dsdas/Kyrsavyia/delotmen.php" />');
...
<script src="./js/delotmen.js"></script>
